...
def add_expense(expense, date, description, amount):
    return expense + [{'tanggal': date, 'deskripsi': description, 'jumlah': amount}]
...
def add_expense_interactively(expense):
    ...
    new_expenses = add_expense(expense, date, description, amount)
...
def view_expenses_report(expense):
    print("\nLaporan Pengeluaran Harian:")
    expenses_report = generate_expenses_report(expense)
...
get_user_input = lambda prompt: int(input(prompt))
